...
// import {
//   Home, Search, MapPin, Maximize, CheckCircle, X, User, Phone,
//   ChevronLeft, ChevronRight, Image as ImageIcon, Send, AlertCircle, Loader
// } from 'lucide-react';
...
//   const [rentData, setRentData] = useState({ fullName: '', phoneNumber: '', address: '' });
//   const [submitting, setSubmitting] = useState(false);
//   const [pageMessage, setPageMessage] = useState(null);   // thông báo ngoài trang
//   const [modalMessage, setModalMessage] = useState(null); // thông báo trong modal
//   const [currentImageIndex, setCurrentImageIndex] = useState(0);
//   const [successSent, setSuccessSent] = useState(false);  // trạng thái đã gửi thành công
...
//         r.status === 'Available' || r.status === 'Trống' ||
//         r.Status === 'Available' || r.Status === 'Trống'
//       );
...
//   // Mở modal và tự điền thông tin user (nếu có)
...
//     // Thử load thông tin profile user hiện tại
//     try {
//       const res = await api.get('/Account/Me');
//       // Điền email vào họ tên tạm (user sẽ sửa lại)
//       setRentData({
//         fullName: '',
//         phoneNumber: '',
//         address: ''
//       });
//     } catch {
//       setRentData({ fullName: '', phoneNumber: '', address: '' });
//     }
...
//       setModalMessage({ type: 'error', text: 'Vui lòng nhập họ tên của bạn!' });
//       setSubmitting(false);
//       return;
//     }
//     if (!rentData.phoneNumber.trim()) {
//       setModalMessage({ type: 'error', text: 'Vui lòng nhập số điện thoại!' });
//       setSubmitting(false);
//       return;
//     }
...
//       // Thành công: hiển thị thông báo thành công trong modal
//       setSuccessSent(true);
//       setModalMessage({
//         type: 'success',
//         text: res.data?.message || 'Gửi yêu cầu thuê phòng thành công! Chủ trọ sẽ liên hệ với bạn sớm nhất.'
//       });
//       fetchRooms(); // Refresh danh sách phòng
//     } catch (err) {
//       const errMsg = err.response?.data?.message || 'Gửi yêu cầu thất bại! Vui lòng kiểm tra lại thông tin.';
...
//   const nextImage = (imgs) => setCurrentImageIndex(prev => (prev + 1) % imgs.length);
//   const prevImage = (imgs) => setCurrentImageIndex(prev => (prev - 1 + imgs.length) % imgs.length);
...
//         <div>
//           <h2 style={{ fontSize: '1.5rem', fontWeight: '700' }}>Tìm phòng thuê</h2>
...
//       {/* Thông báo toàn trang (ngoài modal) */}
//       {pageMessage && (
//         <div style={{
//           backgroundColor: pageMessage.type === 'success' ? '#dcfce7' : '#fee2e2',
//           color: pageMessage.type === 'success' ? '#166534' : '#991b1b',
//           padding: '1.25rem',
//           borderRadius: '0.75rem',
//           display: 'flex', gap: '0.75rem', fontWeight: 600,
//           border: '1px solid currentColor',
//           marginBottom: '1.5rem',
//           alignItems: 'center'
//         }}>
//           {pageMessage.type === 'success' ? <CheckCircle size={22} /> : <AlertCircle size={22} />}
//           {pageMessage.text}
//         </div>
//       )}
...
//             <Loader size={40} className="mx-auto mb-4 text-muted" style={{ animation: 'spin 1s linear infinite' }} />
//             <p className="text-muted">Đang tìm kiếm phòng trống...</p>
//           </div>
//         ) : filteredRooms.length === 0 ? (
//           <div className="col-span-full card text-center py-20">
//             <Home size={64} className="mx-auto mb-4 text-muted" style={{ opacity: 0.1 }} />
//             <p className="text-muted font-medium">Hiện tại không có phòng trống nào phù hợp.</p>
//           </div>
//         ) : filteredRooms.map((room) => {
//           const imgs = getImages(room);
//           const roomName = room.name || room.Name || `Phòng ${room.roomCode || room.RoomCode}`;
//           return (
//             <div key={room.id || room.Id} className="card p-0 overflow-hidden hover-scale shadow-lg border-none bg-white">
//               {/* Ảnh phòng */}
//               <div style={{ aspectRatio: '1 / 1', backgroundColor: '#f8fafc', position: 'relative' }}>
//                 {imgs.length > 0 ? (
//                   <img src={imgs[0]} alt={roomName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
//                 ) : (
//                   <div className="flex flex-col items-center justify-center h-full text-muted" style={{ opacity: 0.2 }}>
//                     <ImageIcon size={64} />
//                   </div>
//                 )}
//                 {imgs.length > 1 && (
...
//                 <h3 style={{ fontSize: '1.375rem', fontWeight: '800', marginBottom: '0.75rem', color: '#1e293b' }}>
//                   {roomName}
//                 </h3>
//                 <div className="space-y-3 mb-6">
//                   <div className="flex items-center gap-2 text-slate-500 text-sm">
//                     <Maximize size={16} /> Diện tích: <strong>{room.area || room.Area} m²</strong>
//                   </div>
//                   <div className="flex items-center gap-2 text-slate-500 text-sm">
//                     <MapPin size={16} /> {room.address || room.Address || 'Liên hệ để biết địa chỉ'}
//                   </div>
...
//                     </span>
//                     <span style={{ fontSize: '0.875rem', fontWeight: '600', color: '#64748b' }}>đ/tháng</span>
//                   </div>
//                 </div>
